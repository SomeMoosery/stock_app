export const addStock = name => {
  return (dispatch, getState) => {
    ...
    return fetch("http://localhost:8000/api/stocks/", {headers, method: "POST", body})
      ...
      .then(res => {
        if (res.status === 201) {
          return dispatch({type: 'ADD_STOCK', stocks: res.data});
...
export const updateStock = (name, index) => {
  return (dispatch, getState) => {
    ...
    return fetch(`http://localhost:8000/api/stocks/${stockId}/`, {headers, method: "PUT", body})
      ...
      .then(res => {
        if (res.status === 200) {
          return dispatch({type: 'UPDATE_STOCK', stock: res.data, index});
        } else if (res.status === 401 || res.status === 403) {
          dispatch({type: "AUTHENTICATION_ERROR", data: res.data});
          throw res.data;
        }
...
export const fetchUserStocks = index => {
  return (dispatch, getState) => {
    let headers = {"Content-Type": "application/json"};
    let {token} = getState().auth;
    ...
    return fetch(`http://localhost:8000/api/profiles/${userId}/stocks`, {headers, })
    .then(res => {
      if (res.status < 500) {
        return res.json().then(data => {
          return {status: res.status, data};
    ...
    .then(res => {
      if (res.status === 200) {
        return dispatch({type: 'FETCH_USER_STOCKS', stocks: res.data});
      } else if (res.status === 401 || res.status === 403) {
        dispatch({type: "AUTHENTICATION_ERROR", data: res.data});
        throw res.data;
      }
...
export const fetchAllStocks = () => {
  return (dispatch, getState) => {
    ...
    return fetch("http://localhost:8000/api/stocks/", {headers, })
      .then(res => {
        if (res.status < 500) {
          return res.json().then(data => {
            return {status: res.status, data};
